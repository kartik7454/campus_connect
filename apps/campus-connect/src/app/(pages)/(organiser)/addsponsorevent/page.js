...
export default function Page() {
    ...
    function addsize2(){
        ...
        setform((prevState)=>{
            return{
                ...prevState,
                offer:[...prevState.faq,{name:"",price:0}] 
            }
        })
    ...
    async function postreq(e){
       ...
       formdata.append("offer",JSON.stringify(form.offer) )
              const response = await fetch('/addsponsorevents',{
                  method:"POST",
...
console.log(form.faq)
  ...
  <h1 className="h-28 bg-slate-100 text-6xl font-bold font-sans text-red-500 text-center italic pt-10">offers</h1>
...
<h1 className="h-28 bg-slate-100 text-6xl font-bold font-sans text-red-500 text-center italic pt-10">FAQ</h1>
<div>{form.faq.map((item,index)=>{
  // eslint-disable-next-line react/jsx-key
  return (<div className="grid gap-6 mb-6 md:grid-cols-1" >
      <div>
          
          <label  className="block mb-2 text-sm font-medium text-gray-900 dark:text-black">question</label>
          <input type="text" onChange={   (e)=>  {form.faq[index].question=e.target.value;console.log(form.faq)} }  className="border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-slate-100 dark:border-gray-600 dark:placeholder-gray-400 dark:text-black dark:focus:ring-blue-500 dark:focus:border-blue-500 "  placeholder={item.name}  required />
      </div>
      <div>
          
          <label  className="block mb-2 text-sm font-medium text-gray-900 dark:text-black">answer</label>
          <input type="text" onChange={   (e)=>  {form.faq[index].answer=e.target.value;console.log(form.faq)} }  className="border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-slate-100 dark:border-gray-600 dark:placeholder-gray-400 dark:text-black dark:focus:ring-blue-500 dark:focus:border-blue-500 "  placeholder={item.name}  required />
      </div>
      <div>
